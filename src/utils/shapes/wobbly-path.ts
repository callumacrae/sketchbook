 ...
 */
export default function generatePath(start: Coord, end: Coord, config: Config) {
  ...
  do {
    ...
    const perfect = Vector.between(currentPoint, end);
  ...
  } while (distBetween(currentPoint, end) > config.SEGMENT_LENGTH && maxRuns--);
